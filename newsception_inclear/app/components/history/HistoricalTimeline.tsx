...
import { generateHistoricalContext, type HistoricalContext } from "@/lib/insights";
...
export default function HistoricalTimeline({ topic, currentArticleId }: HistoricalTimelineProps) {
  const [timelineData, setTimelineData] = useState<HistoricalContext>(EMPTY_CONTEXT);
  ...
  useEffect(() => {
    ...
    const loadContext = async () => {
      ...
      setExpanded(false);
      try {
        const context = await generateHistoricalContext(topic);
        if (!isMounted) return;
        setTimelineData(context);
      } catch (error) {
        console.error("Error loading historical context:", error);
        if (isMounted) {
          setTimelineData(EMPTY_CONTEXT);
        }
      } finally {
        if (isMounted) {
          setIsLoading(false);
  ...
  if (!timelineData.timeline_events.length) {
    return (
      <div className="bg-white dark:bg-[#1a1a1a] border-2 border-[#1a1a1a] dark:border-[#333] p-6 text-center">
        <Clock className="w-6 h-6 mx-auto mb-3 text-[#d4af37]" />
        <p className="font-serif text-sm text-gray-600 dark:text-gray-400">
          No historical events available for this topic yet.
        </p>
      </div>
    );
  }
  ...
  return (
    ...
    >
      <div className="flex items-start gap-4 mb-6">
        <div className="w-12 h-12 border-2 border-[#d4af37] flex items-center justify-center shrink-0">
      ...
      <div className="relative">
        <div className="absolute left-6 top-0 bottom-0 w-0.5 bg-[#d4af37]" />
          ...
          {displayEvents.map((event, idx) => (
            ...
            >
              <div className="absolute left-0 top-1 w-12 h-12 rounded-full border-4 border-[#d4af37] bg-white dark:bg-[#1a1a1a] flex items-center justify-center">
                <span className="font-serif font-bold text-xs text-[#d4af37]">{idx + 1}</span>
              </div>
      ...
      {timelineData.timeline_events.length > 3 && (
        <div className="mt-6 text-center">
          <Button onClick={() => setExpanded(!expanded)} variant="outline" className="font-serif">
            {expanded ? (
              <>
      ...
      {timelineData.key_developments.length > 0 && (
        <div className="mt-8 pt-8 border-t-2 border-gray-200 dark:border-gray-800">
          ...
          <ul className="space-y-2">
            {timelineData.key_developments.map((item, idx) => (
              <li key={`${item}-${idx}`} className="flex items-start gap-3 font-serif text-sm text-gray-700 dark:text-gray-300">
      ...
      {timelineData.related_topics.length > 0 && (
        <div className="mt-6 bg-gray-50 dark:bg-[#0a0a0a] p-4 rounded">
          <p className="font-serif text-xs text-gray-600 dark:text-gray-400 mb-2">Related Historical Topics:</p>
          <div className="flex flex-wrap gap-2">
            {timelineData.related_topics.map((relatedTopic, idx) => (
              <span
                key={`${relatedTopic}-${idx}`}
                className="px-3 py-1 text-xs font-serif border border-[#d4af37] text-[#d4af37] hover:bg-[#d4af37]/10 transition-colors"
              >
                {relatedTopic}
              </span>
            ))}
