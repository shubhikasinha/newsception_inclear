  ...
  async function joinRoom(chosenSide) {
    ...
    try {
      ...
      const res = await fetch(
        `/api/livekit-token?user=${userId}-${chosenSide}&room=${debateId}`
      );
    ...
    <div className="min-h-screen bg-[#fafafa] font-serif">
      {!joined ? (
        <div className="min-h-screen flex items-center justify-center p-4">
          ...
          >
            <div className="mb-6">
              <h1 className="text-3xl font-bold mb-2 text-[#1a1a1a] tracking-wide">
                {debateTopic}
              </h1>
              <div className="w-24 h-1 bg-[#d4af37] mx-auto mb-4"></div>
              <p className="text-[#666] font-serif text-sm">
                Choose your side and join the debate
              </p>
            </div>
